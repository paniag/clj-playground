...
;nil if the location is not a valid move
;if the location is a valid move:
; pair [loc anchors]
; where anchors is a dictionary mapping affected
; directions to location of next piece of this player
; in that direction
...
;                  {}
;                  (for [dir (range 8)]
;                    (valid-directed-move? board player loc dir)))]
;    (when (< 0 (count anchors))
;      {:start loc :anchors anchors})))
